...
class Role:
    ...
    def __init__(self, name, role, weapon, life_value=100, money=15000):
        ...
        self.role = role
        self.weapon = weapon
        self.__life_value = life_value  #私有属性，前面加两根下滑线
        self.money = money
    def __del__(self):  #析构函数，在实例释放、销毁的时候自动执行的，通常用于做一些收尾工作， 如关闭一些数据库连接，关闭打开的临时文件
    ...
    def show_status(self):
        print("name:%s weapon:%s life_val:%s" %(self.name,
                                                 self.weapon,
                                                self.__life_value))
    def __shot(self): # 类的方法，功能 （动态属性），#私有方法，前面加两根下滑线
        print("shooting...")
...
del r1  #在此删除了r1因此析构函数__del__()会执行
